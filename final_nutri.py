...
st.set_page_config(page_title="Nutrient Dosing & Fertigation Designer", layout="wide")
...
# UI
# -----------------------------
st.title("🌱 Nutrient Dosing, Fertigation & Crop-Water Designer")
st.caption("Balanced fertilizer combinations, ET-based water requirement, and concentrated Stock A/Stock B preparation.")
...
for number, label in enumerate(STEP_LABELS, start=1):
    if number < wizard_step:
        marker = "✓"
    elif number == wizard_step:
        marker = "●"
    else:
        marker = "🔒"
    step_status.append(f"{marker} {number}. {label}")
st.caption("  |  ".join(step_status))

if wizard_step == 1:
    st.subheader("Project, crop and cultivation system")
    ...
    st.subheader("Select or customize the elemental nutrient formulation")
    ...
    st.subheader("Choose fertilizer chemicals")
    st.info(
    ...
    st.subheader("Validated Balance and Stock Preparation")
    ...
    c1, c2 = st.columns(2)
    with c1:
        stock_volume_A = st.number_input(
            "Stock A final volume (L)", min_value=0.1, value=10.0,
            key="wizard_stock_volume_A"
        )
    with c2:
        stock_volume_B = st.number_input(
            "Stock B final volume (L)", min_value=0.1, value=10.0,
            key="wizard_stock_volume_B"
        )
    ...
    for chem, mg_l in zip(selected, x):
        ...
        kg_season_batch = mg_l * seasonal_solution_volume / 1_000_000
        ...
        fert_rows.append({
            ...
            "Estimated kg/crop period (all fresh supply dosed)": kg_season_batch,
            "Stock tank": tank
        })
        stock_rows.append({
            "Stock": f"Stock {tank}",
            ...
            "Stock volume (L)": stock_vol,
            "Mass to add (g)": stock_mass_g
        })
    ...
    st.write(
        f"Prepare Stock A and Stock B separately at **{stock_factor}× concentration**. "
        f"At a 1:{stock_factor} injector setting, each stock is injected separately into flowing water."
    )
    st.warning(
